...
const EXISTING_PLUS_OAUTH_JSON_STEP_DEFINITIONS = self.MultiPageStepDefinitions?.getSteps?.({
  activeFlowId: DEFAULT_ACTIVE_FLOW_ID,
...
const ALL_STEP_DEFINITIONS = self.MultiPageStepDefinitions?.getAllSteps?.({
  ...
  ...EXISTING_PLUS_OAUTH_JSON_STEP_DEFINITIONS,
];
...
const HOTMAIL_ACCOUNTS_EXHAUSTED_ERROR_PREFIX = 'HOTMAIL_ACCOUNTS_EXHAUSTED::';
...
const EXISTING_PLUS_SESSION_CLEAR_DOMAINS = [
  'auth.openai.com',
  'auth0.openai.com',
  'accounts.openai.com',
  'chatgpt.com',
...
const DEFAULT_PHONE_CODE_WAIT_SECONDS = 45;
...
const DEFAULT_PHONE_CODE_POLL_INTERVAL_SECONDS = 3;
...
const HOTMAIL_LOCAL_HELPER_TIMEOUT_MS = 90000;
...
const DEFAULT_PHONE_SMS_PROVIDER = PHONE_SMS_PROVIDER_SMSBOWER;
...
const DEFAULT_SMSBOWER_BASE_URL = 'https://smsbower.app/stubs/handler_api.php';
...
const FIVE_SIM_COUNTRY_ID = 'colombia';
const FIVE_SIM_COUNTRY_LABEL = '哥伦比亚 (Colombia)';
const FIVE_SIM_SUPPORTED_COUNTRY_IDS = [
  'colombia',
  'chile',
  'brazil',
  'england',
...
const HERO_SMS_SUPPORTED_COUNTRY_IDS = [33, 151, 73, 16, 31, 4, 6, 187, 46, 117, 8, 10, 15, 32, 43, 52, 78, 182];
...
const HERO_SMS_COUNTRY_BY_PHONE_PREFIX = Object.freeze([
  ...
  { prefix: '27', id: 31, label: 'South Africa' },
...
const PERSISTED_SETTING_DEFAULTS = {
  ...
  profileRunnerProxyLocked: false,
...
const DEFAULT_STATE = {
  ...
  existingPlusCurrentAccountId: null, // 已有 Plus OAuth 导出当前账号 ID。
  existingPlusCurrentEmail: '', // 已有 Plus OAuth 导出当前邮箱。
  existingPlusJsonFilePath: '', // 已有 Plus OAuth 导出生成的 JSON 路径。
  existingPlusAccountStatuses: {}, // 已有 Plus OAuth 导出账号状态。
...
function normalizeAutoRunTimerPlan(plan) {
  ...
  const targetSuccessMode = Boolean(plan.targetSuccessMode);
...
function normalizeAutoRunTimerPlanFromState(state = {}) {
  ...
  return normalizeAutoRunTimerPlan({
    ...
    targetSuccessMode: state.scheduledAutoRunPlan?.targetSuccessMode ?? state.targetSuccessMode,
...
function normalizePanelMode(value = '') {
  const normalized = String(value || '').trim().toLowerCase();
  if (normalized === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE) {
    return EXISTING_PLUS_OAUTH_JSON_PANEL_MODE;
  }
...
async function upsertHotmailAccount(input) {
  ...
  const hasRefreshToken = Boolean(String(input?.refreshToken ?? existing?.refreshToken ?? '').trim());
  const normalized = normalizeHotmailAccount({
    ...(existing || {}),
    ...(credentialsChanged ? {
      status: hasRefreshToken ? 'authorized' : 'pending',
      used: false,
      lastAuthAt: 0,
      lastError: '',
    } : {}),
...
function isAuthorizedHotmailRunAccount(candidate) {
  return Boolean(candidate)
    && (candidate.status === 'authorized' || (
      ...
      && Boolean(String(candidate.refreshToken || '').trim())
    ))
    && !candidate.used
    && Boolean(String(candidate.refreshToken || '').trim());
}
...
async function ensureHotmailAccountForFlow(options = {}) {
  ...
  const isReusableAuthorizedHotmailAccount = (account) => Boolean(account)
    && (account.status === 'authorized' || account.status !== 'error')
  ...
  const tokenAccounts = accounts.filter((account) => String(account?.refreshToken || '').trim()).length;
  const usedAccounts = accounts.filter((account) => Boolean(account?.used)).length;
  const errorAccounts = accounts.filter((account) => account?.status === 'error').length;
  throw new Error(
    `没有可用的 Hotmail 账号。请先在侧边栏添加至少一个带刷新令牌（refresh token）的账号。当前池：${totalAccounts} 条，带 refreshToken：${tokenAccounts} 条，已用：${usedAccounts} 条，异常：${errorAccounts} 条。`
  );
}
...
function isHotmailRefreshTokenInvalidError(error) {
  const message = String(error?.message || error || '');
  return /invalid_grant|refresh_token['"]?\s*(?:or\s+['"]?assertion['"]?)?\s+is\s+not\s+valid|provided value for the input parameter ['"]?refresh_token/i.test(message);
}

async function markHotmailAccountInvalidIfTokenExpired(accountId, error, options = {}) {
  if (!accountId || !isHotmailRefreshTokenInvalidError(error)) {
    return false;
  }
  const reason = String(error?.message || error || 'refresh token invalid');
  await patchHotmailAccount(accountId, {
    used: true,
    status: 'failed',
    lastUsedAt: Date.now(),
    lastError: 'refresh_token_invalid',
  }, {
    preserveCurrentSelection: true,
  }).catch(() => null);
  await addLog(
    `${options.logPrefix || 'Hotmail 账号'} refreshToken 已失效，已标记为已用/不可用。${reason}`,
    options.level || 'warn',
    options.logOptions || {}
  ).catch(() => null);
  return true;
...
async function ensureHotmailMailboxReadyForAutoRunRound(options = {}) {
  ...
  while (true) {
    ...
    if (!remainingAuthorizedAccounts.length && !remainingPendingAccounts.length) {
      if (lastError) {
        throw new Error(`${HOTMAIL_ACCOUNTS_EXHAUSTED_ERROR_PREFIX}自动运行${buildRoundLabel()}开始前未找到可通过校验的 Hotmail 账号：${lastError.message}`);
      }
      throw new Error(`${HOTMAIL_ACCOUNTS_EXHAUSTED_ERROR_PREFIX}没有可用的 Hotmail 账号。请先在侧边栏添加至少一个带刷新令牌（refresh token）的账号。`);
    }
    ...
    try {
      ...
      await markHotmailAccountInvalidIfTokenExpired(account.id, error, {
        logPrefix: `自动运行${buildRoundLabel()}开始前校验 Hotmail 账号 ${account.email} 失败：`,
        logOptions: {},
      });
      await addLog(
  ...
  if (normalizedPlan.kind === AUTO_RUN_TIMER_KIND_SCHEDULED_START) {
    return {
      loopOptions: {
        ...
        targetSuccessMode: normalizedPlan.targetSuccessMode,
  ...
  try {
    ...
    await broadcastAutoRunStatus(
      ...
      resumeOptions.statusPayload,
      {
        ...
        targetSuccessMode: plan.targetSuccessMode,
...
async function scheduleAutoRun(totalRuns, options = {}) {
  ...
  const timerPlan = normalizeAutoRunTimerPlan({
    ...
    targetSuccessMode: options.targetSuccessMode,
  ...
  await persistAutoRunTimerPlan(timerPlan, {
    ...
    targetSuccessMode: timerPlan.targetSuccessMode,
...
async function handleStepData(step, payload) {
  ...
  if (payload.localhostUrl) {
    if (!isLocalhostOAuthCallbackUrl(payload.localhostUrl)) {
      throw new Error(`节点返回了无效的 localhost OAuth 回调地址。`);
    }
    await setState({
...
async function failNodeFromBackground(nodeId, errorLike = '未知错误') {
  ...
  const latestState = await getState();
  if (
    getPanelMode(latestState) === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE
    && latestState.existingPlusCurrentAccountId
  ) {
    await runBestEffort('mark existing plus account failed', () => setExistingPlusAccountStatus(
      latestState.existingPlusCurrentAccountId,
      {
        status: 'failed',
        email: latestState.existingPlusCurrentEmail || latestState.email || '',
        reason: message,
        finishedAt: Date.now(),
      }
    ));
...
const AUTO_RUN_NODE_DELAYS = Object.freeze({
  'existing-plus-prepare-oauth': 1000,
  'open-chatgpt': 2000,
...
async function disableLegacyIpProxyFeatureRuntime() {
  const restoreProfileRunnerProxyIfLocked = async () => {
    const latest = await getState().catch(() => ({}));
    if (!Boolean(latest?.profileRunnerProxyLocked)) {
      return false;
    }
    const restoredState = {
      ...latest,
    ...
    if (typeof applyIpProxySettingsFromState === 'function') {
      await applyIpProxySettingsFromState(restoredState, {
        skipExitProbe: true,
  ...
  if (await restoreProfileRunnerProxyIfLocked()) {
    return;
  }
  ...
  const state = await getState().catch(() => ({}));
  if (Boolean(state?.profileRunnerProxyLocked)) {
    await restoreProfileRunnerProxyIfLocked();
    return;
  }
  const patch = {
    ipProxyEnabled: false,
...
const autoRunController = self.MultiPageBackgroundAutoRunController?.createAutoRunController({
  ...
  getRunningNodeIds,
  getState,
  getWorkflowNodeIds: (state = {}) => getNodeIdsForState(state),
...
function normalizeExistingPlusEmail(value = '') {
  return String(value || '').trim().toLowerCase();
}

function buildExistingPlusAccountId(email = '') {
  const normalizedEmail = normalizeExistingPlusEmail(email);
  const safe = normalizedEmail.replace(/[^a-z0-9._+-]+/g, '-').replace(/^-+|-+$/g, '');
  return `existing-plus-${safe || crypto.randomUUID()}`;
}

function parseExistingPlusAccountsText(text = '') {
  ...
  rows.forEach((line, index) => {
    ...
    if (parts.length !== 3) {
      errors.push(`第 ${lineNumber} 行格式错误，应为 email----clientId----mailRefreshToken。`);
      return;
    }
    const [rawEmail, clientId, refreshToken] = parts;
    const email = normalizeExistingPlusEmail(rawEmail);
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      errors.push(`第 ${lineNumber} 行邮箱无效：${rawEmail || '(空)'}`);
      return;
    }
    if (!clientId) {
      errors.push(`第 ${lineNumber} 行缺少 clientId。`);
      return;
    }
    if (!refreshToken) {
      errors.push(`第 ${lineNumber} 行缺少 mailRefreshToken。`);
      return;
    }
    if (seen.has(email)) {
      errors.push(`第 ${lineNumber} 行邮箱重复：${email}`);
      return;
    }
    seen.add(email);
    accounts.push({
      id: buildExistingPlusAccountId(email),
      email,
      clientId,
      refreshToken,
      lineNumber,
...
function resolveExistingPlusAccountForRun(accounts = [], state = {}) {
  if (!accounts.length) {
    return null;
  }
  const runIndex = Math.max(0, Math.floor(Number(state?.autoRunCurrentRun) || 1) - 1);
  if (accounts[runIndex]) {
    return accounts[runIndex];
  }
  const statuses = state?.existingPlusAccountStatuses && typeof state.existingPlusAccountStatuses === 'object'
    ? state.existingPlusAccountStatuses
    : {};
  return accounts.find((account) => statuses[account.id]?.status !== 'success') || accounts[0];
}

function getExistingPlusLocalApi() {
  const factory = self.MultiPageBackgroundLocalCliProxyApi?.createLocalCliProxyApi;
  if (typeof factory !== 'function') {
    throw new Error('本地 OAuth JSON 模块未加载。');
  }
  return factory({
...
function normalizeExistingPlusOutputDir(value = '') {
  const raw = String(value || '').trim();
  if (!raw) return DEFAULT_EXISTING_PLUS_JSON_OUTPUT_DIR;
  const trimmed = raw.replace(/[\\/]+$/g, '');
  if (trimmed === DEFAULT_EXISTING_PLUS_JSON_OUTPUT_DIR) {
    return DEFAULT_EXISTING_PLUS_JSON_OUTPUT_DIR;
  }
  if (raw.startsWith('/') || /^[A-Za-z]:[\\/]/.test(raw) || trimmed) {
    return DEFAULT_EXISTING_PLUS_JSON_OUTPUT_DIR;
  }
  return '';
}

function normalizeExistingPlusCookieDomain(value = '') {
  return String(value || '').trim().replace(/^\.+/, '').toLowerCase();
}

function shouldClearExistingPlusCookie(cookie = {}) {
  const domain = normalizeExistingPlusCookieDomain(cookie.domain);
  if (!domain) return false;
  return EXISTING_PLUS_SESSION_CLEAR_DOMAINS.some((target) => (
    domain === target || domain.endsWith(`.${target}`)
  ));
}

function buildExistingPlusCookieRemovalUrl(cookie = {}) {
  const host = normalizeExistingPlusCookieDomain(cookie.domain);
  const rawPath = String(cookie.path || '/');
  const path = rawPath.startsWith('/') ? rawPath : `/${rawPath}`;
  return `https://${host}${path}`;
}

function isExistingPlusOpenAiSessionUrl(value = '') {
  try {
...
async function collectExistingPlusSessionCookies() {
  ...
  for (const store of stores) {
    ...
    for (const cookie of batch || []) {
      if (!shouldClearExistingPlusCookie(cookie)) continue;
      const key = [
...
async function removeExistingPlusSessionCookie(cookie = {}) {
  if (!chrome?.cookies?.remove) {
    return false;
  }
  const details = {
    url: buildExistingPlusCookieRemovalUrl(cookie),
    name: cookie.name,
  };
...
async function clearExistingPlusOpenAiTabStorage() {
  if (!chrome?.tabs?.query || !chrome?.scripting?.executeScript) {
    return { tabCount: 0, frameCount: 0 };
  }
  const tabs = (await chrome.tabs.query({}).catch(() => []))
    .filter((tab) => Number.isInteger(tab?.id) && isExistingPlusOpenAiSessionUrl(tab?.url || ''));
  let frameCount = 0;
  for (const tab of tabs) {
    try {
      ...
      // Some pages/frames may reject script injection while navigating; browsingData below is the fallback.
    }
  }
  return { tabCount: tabs.length, frameCount };
}

async function clearExistingPlusOpenAiSessionBeforeRun(options = {}) {
  const nodeId = options?.nodeId || 'existing-plus-prepare-oauth';
  await addLog('开始新账号前清理 OpenAI / ChatGPT cookies 与本地存储...', 'info', { nodeId });
  let removedCount = 0;
  let candidateCount = 0;
  if (chrome?.cookies?.getAll && chrome?.cookies?.remove) {
    const cookies = await collectExistingPlusSessionCookies();
    candidateCount = cookies.length;
    for (const cookie of cookies) {
      if (await removeExistingPlusSessionCookie(cookie)) {
        removedCount += 1;
      }
    }
  } else {
    await addLog('当前浏览器不支持 cookies API，跳过 cookie 精确清理。', 'warn', { nodeId });
  }

  const storageCleanup = await clearExistingPlusOpenAiTabStorage();
  if (chrome?.browsingData?.remove) {
    try {
      await chrome.browsingData.remove(
        { since: 0, origins: EXISTING_PLUS_SESSION_CLEAR_ORIGINS },
        {
          cookies: true,
          localStorage: true,
          indexedDB: true,
      ...
      await addLog(`browsingData 清理 OpenAI / ChatGPT 状态失败：${getErrorMessage(error)}`, 'warn', { nodeId });
    }
  }
  await addLog(
    `已清理 OpenAI / ChatGPT 状态：cookies ${removedCount}/${candidateCount} 个，storage 标签页 ${storageCleanup.tabCount} 个 / frame ${storageCleanup.frameCount} 个。`,
    'ok',
    { nodeId }
  );
}

function buildExistingPlusJsonFilePath(outputDir, fileName) {
  const directory = normalizeExistingPlusOutputDir(outputDir);
  const safeName = String(fileName || '')
    .trim()
    .split(/[\\/]/)
    .filter(Boolean)
    .pop();
  if (!directory || !safeName) {
    return '';
  }
  return `${directory}/${safeName}`;
}

function buildExistingPlusSub2ApiJsonFileName(email = '') {
  const safeEmail = sanitizeSessionExportFileSegment(
    normalizeExistingPlusEmail(email),
    'existing-plus'
  );
  return `sub2api-${safeEmail}.json`;
}

async function setExistingPlusAccountStatus(accountId, updates = {}) {
  const state = await getState();
  const statuses = state.existingPlusAccountStatuses && typeof state.existingPlusAccountStatuses === 'object'
    ? state.existingPlusAccountStatuses
    : {};
  ...
  await setState({ existingPlusAccountStatuses: nextStatuses });
  broadcastDataUpdate({ existingPlusAccountStatuses: nextStatuses });
  return nextStatus;
}
...
async function pickVerifiedExistingPlusHotmailAccount(accounts = [], state = {}, options = {}) {
  ...
  for (const candidate of candidates) {
    const email = normalizeExistingPlusEmail(candidate.email);
    try {
      await addLog(`微软邮箱账户池：正在自动校验 ${email || candidate.id} 的收信能力...`, 'info', { nodeId });
      const verified = await verifyHotmailAccount(candidate.id);
      const verifiedAccount = normalizeHotmailAccount(verified?.account || candidate);
      await addLog(
        `微软邮箱账户池：${verifiedAccount.email || email || candidate.id} 自动校验通过，INBOX 当前 ${verified?.messageCount ?? 0} 封邮件。`,
        'ok',
        { nodeId }
      );
      ...
      failures.push(`${email || candidate.id}: ${reason}`);
      await markHotmailAccountInvalidIfTokenExpired(candidate.id, error, {
        logPrefix: `微软邮箱账户池：${email || candidate.id}`,
        logOptions: { nodeId },
      });
      await setExistingPlusAccountStatus(candidate.id, {
        status: 'failed',
        email,
        reason: `微软邮箱自动校验失败：${reason}`,
        finishedAt: Date.now(),
      }).catch(() => {});
      await addLog(`微软邮箱账户池：${email || candidate.id} 自动校验失败，尝试下一个账号。${reason}`, 'warn', { nodeId });
    }
  }

  const suffix = failures.length ? `失败详情：${failures.join(' | ')}` : '';
  throw new Error(`${HOTMAIL_ACCOUNTS_EXHAUSTED_ERROR_PREFIX}没有可通过自动校验的微软邮箱账号。请确认账号带 refreshToken 且本地助手可读取邮箱。${suffix}`.trim());
}

async function executeExistingPlusPrepareOauth(state = {}) {
  const latestState = await getState();
  await clearExistingPlusOpenAiSessionBeforeRun({
    nodeId: state?.nodeId || 'existing-plus-prepare-oauth',
  });
  if (!normalizeExistingPlusOutputDir(latestState.existingPlusJsonOutputDir)) {
    throw new Error('生成 SUB2API JSON 输出目录失败。');
  }
  ...
  const selectedHotmailAccount = await setCurrentHotmailAccount(verifiedHotmailAccount.id, {
    markUsed: false,
    syncEmail: false,
  });
  const accountId = selectedHotmailAccount.id;
  const accountEmail = normalizeExistingPlusEmail(selectedHotmailAccount.email);
  ...
  await setState({
    existingPlusCurrentAccountId: accountId,
    existingPlusCurrentEmail: accountEmail,
    existingPlusJsonFilePath: '',
    ...
    ipProxyEnabled: keepProfileRunnerProxy,
    oauthUrl: authRequest.oauthUrl,
    localhostUrl: null,
    localCpaJsonOAuthState: authRequest.oauthState,
    localCpaJsonPkceCodes: authRequest.pkceCodes,
    lastLoginCode: null,
    currentPhoneVerificationCode: '',
    signupPhoneVerificationRequestedAt: null,
    signupPhoneVerificationPurpose: '',
  });
  await setExistingPlusAccountStatus(accountId, {
    status: 'running',
    email: accountEmail,
    reason: '',
    phoneNumber: '',
  ...
  await addLog(`已从微软邮箱账户池选择 ${accountEmail}，正在使用邮箱验证码登录 OAuth。`, 'ok', {
    nodeId: state?.nodeId || 'existing-plus-prepare-oauth',
  });
  await completeNodeFromBackground(state?.nodeId || 'existing-plus-prepare-oauth', {
    email: accountEmail,
    oauthUrl: authRequest.oauthUrl,
    localCpaJsonOAuthState: authRequest.oauthState,
    localCpaJsonPkceCodes: authRequest.pkceCodes,
...
function formatExistingPlusLocalHelperFetchError(endpoint, helperBaseUrl, error) {
  const originalMessage = String(error?.message || error || 'Failed to fetch').trim();
  return `本地 helper 请求失败：无法连接 ${endpoint}。请先运行 start-hotmail-helper.command，并确认本地助手地址为 ${helperBaseUrl}。原始错误：${originalMessage}`;
}

async function saveExistingPlusJsonArtifactViaHelper(helperBaseUrl, artifact) {
  ...
  try {
    ...
    throw new Error(formatExistingPlusLocalHelperFetchError(endpoint, helperBaseUrl, error));
  }
  ...
  if (!response.ok || payload?.ok === false) {
    throw new Error(String(payload?.error || '').trim() || `本地 helper 写入失败（HTTP ${response.status}）。`);
  }
  return {
    ...artifact,
    filePath: String(payload?.filePath || '').trim() || artifact.filePath,
  };
}

async function executeExistingPlusSaveJson(state = {}) {
  const latestState = {
    ...(await getState().catch(() => ({}))),
    ...(state || {}),
  };
  const accountId = String(latestState.existingPlusCurrentAccountId || '').trim();
  const email = normalizeExistingPlusEmail(latestState.existingPlusCurrentEmail || latestState.email);
  const helperBaseUrl = normalizeHotmailLocalBaseUrl(latestState.hotmailLocalBaseUrl);
  const outputDir = normalizeExistingPlusOutputDir(latestState.existingPlusJsonOutputDir);
  if (!latestState.localhostUrl || !isLocalhostOAuthCallbackUrl(latestState.localhostUrl)) {
    throw new Error('缺少有效的 localhost OAuth 回调地址，请先完成 OAuth 授权确认。');
  }
  if (!latestState.localCpaJsonPkceCodes?.codeVerifier) {
    throw new Error('缺少 PKCE 会话信息，请重新执行准备 OAuth 步骤。');
  }
  if (!helperBaseUrl) {
    throw new Error('尚未配置 Hotmail 本地助手地址。');
  }
  if (!outputDir) {
    throw new Error('JSON 输出目录为空。');
  }

  const api = getExistingPlusLocalApi();
  await addLog(`正在为 ${email || '当前账号'} 交换 refresh_token 并生成 SUB2API JSON...`, 'info', {
    nodeId: state?.nodeId || 'existing-plus-save-json',
  });
  const artifact = await api.exchangeCallbackToAuthArtifact({
    callbackUrl: latestState.localhostUrl,
    expectedState: latestState.localCpaJsonOAuthState,
    pkceCodes: latestState.localCpaJsonPkceCodes,
    outputDir,
    fileNamePrefix: 'sub2api',
    sourceName: 'Existing Plus OAuth',
  });
  if (!artifact?.authJson?.refresh_token) {
    throw new Error('OAuth token 交换成功但未返回 refresh_token，已停止保存。');
  }

  for (const warning of Array.isArray(artifact.warnings) ? artifact.warnings : []) {
    await addLog(warning, 'warn', { nodeId: state?.nodeId || 'existing-plus-save-json' });
  }
  const artifactForSave = {
    ...artifact,
    directoryPath: outputDir,
    fileName: buildExistingPlusSub2ApiJsonFileName(email || artifact.authJson?.email),
    filePath: buildExistingPlusJsonFilePath(
      outputDir,
      buildExistingPlusSub2ApiJsonFileName(email || artifact.authJson?.email)
    ),
  };
  if (!artifactForSave.filePath) {
    throw new Error('生成 SUB2API JSON 保存路径失败。');
  }
  const saved = await saveExistingPlusJsonArtifactViaHelper(helperBaseUrl, artifactForSave);
  await setState({ existingPlusJsonFilePath: saved.filePath });
  if (accountId) {
    const phoneNumber = String(
      latestState.phoneNumber
    ...
    await patchHotmailAccount(accountId, {
      ...
      preserveCurrentSelection: true,
    }).catch((error) => addLog(`微软邮箱账户池：标记 ${email || accountId} 已用失败：${error?.message || error}`, 'warn'));
  }
  broadcastDataUpdate({ existingPlusJsonFilePath: saved.filePath });
  await addLog(`SUB2API JSON 已保存：${saved.filePath}`, 'ok', {
    nodeId: state?.nodeId || 'existing-plus-save-json',
  });
  await completeNodeFromBackground(state?.nodeId || 'existing-plus-save-json', {
    localCpaJsonFilePath: saved.filePath,
    existingPlusJsonFilePath: saved.filePath,
  });
}

const stepExecutorsByKey = {
  'existing-plus-prepare-oauth': (state) => executeExistingPlusPrepareOauth(state),
  ...
  'existing-plus-save-json': (state) => executeExistingPlusSaveJson(state),
};
...
const existingPlusOauthJsonStepRegistry = buildStepRegistry(EXISTING_PLUS_OAUTH_JSON_STEP_DEFINITIONS);
...
function getStepRegistryForState(state = {}) {
  ...
  if (getPanelMode(state) === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE) {
    return existingPlusOauthJsonStepRegistry;
  }
...
function getMailConfig(state) {
  const provider = normalizeMailProvider(state.mailProvider || HOTMAIL_PROVIDER);
  if (provider === 'custom') {
...
async function refreshOAuthUrlBeforeStep6(state, options = {}) {
  ...
  if (getPanelMode(state) === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE) {
    ...
    if (!oauthUrl) {
      throw new Error(`步骤 ${visibleStep}：已有 Plus OAuth JSON 模式缺少本地 OAuth 链接，请先执行“准备 Plus 邮箱 OAuth”。`);
    }
    await addLog(`已有 Plus OAuth JSON 模式：直接使用本地 PKCE OAuth 链接登录，不请求 CPA 地址。`, 'info', {
      step: visibleStep,
      stepKey: 'oauth-login',
    });
    return oauthUrl;
