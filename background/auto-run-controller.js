(function attachBackgroundAutoRunController(root, factory) {
  ...
  function createAutoRunController(deps = {}) {
    ...
    function getFirstWorkflowNode(state = {}) {
      if (typeof getWorkflowNodeIds === 'function') {
        const nodeIds = getWorkflowNodeIds(state);
        if (Array.isArray(nodeIds) && nodeIds.length) {
          return String(nodeIds[0] || '').trim() || null;
        }
      }
      const firstUnfinished = getFirstUnfinishedWorkflowNode(state);
      return firstUnfinished ? String(firstUnfinished || '').trim() : null;
    }
    ...
    function serializeAutoRunRoundSummaries(totalRuns, roundSummaries = []) {
      const summaryCount = Math.max(totalRuns, Array.isArray(roundSummaries) ? roundSummaries.length : 0);
      return buildAutoRunRoundSummaries(summaryCount, roundSummaries).map((summary) => ({
        ...summary,
    ...
    async function autoRunLoop(totalRuns, options = {}) {
      ...
      const targetSuccessMode = Boolean(options.targetSuccessMode);
      ...
      for (
        let targetRun = resumeCurrentRun;
        targetSuccessMode ? successfulRuns < totalRuns : targetRun <= totalRuns;
        targetRun += 1
      ) {
        while (roundSummaries.length < targetRun) {
          roundSummaries.push(createAutoRunRoundSummary(roundSummaries.length + 1));
        }
        ...
        while (attemptRun <= maxAttemptsForRound) {
          ...
          const defaultStartNodeId = typeof runAutoSequenceFromNode === 'function'
            ? (getFirstWorkflowNode(await getState()) || 'open-chatgpt')
            : 1;
          ...
          try {
            ...
            await addLog(
              targetSuccessMode
                ? `=== 第 ${targetRun} 轮完成（第 ${attemptRun} 次尝试成功），目标进度 ${successfulRuns}/${totalRuns} ===`
                : `=== 第 ${targetRun}/${totalRuns} 轮完成（第 ${attemptRun} 次尝试成功）===`,
              'ok'
            );
            break;
          } catch (err) {
            ...
            const blockedByHotmailAccountsExhausted = /HOTMAIL_ACCOUNTS_EXHAUSTED::|没有可用的\s*(?:Hotmail|微软邮箱)|未找到可通过校验的\s*(?:Hotmail|微软邮箱)|没有可通过自动校验的微软邮箱账号|refresh[_\s-]?token.*(?:invalid|not\s+valid)|invalid_grant/i.test(reason);
            ...
            if (blockedByAccountDeactivated) {
              ...
              cancelPendingCommands('当前轮因账号已删除或停用已终止，准备切换下一个账号。');
              await broadcastStopToContentScripts();
              await addLog(`第 ${targetRun}/${totalRuns} 轮账号已删除或停用，本轮将直接失败并切换下一个账号。`, 'warn');
              await addLog(
                targetRun < totalRuns
                  ? `第 ${targetRun}/${totalRuns} 轮因账号停用提前结束，自动流程将继续下一轮。`
                  : `第 ${targetRun}/${totalRuns} 轮因账号停用提前结束，已无后续轮次，本次自动运行结束。`,
                'warn'
              );
              forceFreshTabsNextRun = true;
              break;
            ...
            if (blockedByAddPhone) {
              ...
              await broadcastStopToContentScripts();
              if (!autoRunSkipFailures && !targetSuccessMode) {
                await addLog(
            ...
            if (blockedByHotmailAccountsExhausted) {
              roundSummary.status = 'failed';
              roundSummary.finalFailureReason = reason;
              await setState({
                autoRunRoundSummaries: serializeAutoRunRoundSummaries(totalRuns, roundSummaries),
              ...
              await appendRoundRecordIfNeeded('failed', reason, err);
              cancelPendingCommands('当前轮因 Hotmail 账号池无可用账号已终止。');
              await broadcastStopToContentScripts();
              await addLog(
                `第 ${targetRun}/${totalRuns} 轮 Hotmail 账号池已无可用账号或 refreshToken 已失效，目标成功模式也已停止；请导入新的有效微软邮箱账号后再继续。`,
                'warn'
              );
              stoppedEarly = true;
              await broadcastAutoRunStatus('stopped', {
      ...
      const summaryTotalRuns = targetSuccessMode ? Math.max(totalRuns, roundSummaries.length) : totalRuns;
      await setState({
        autoRunRoundSummaries: serializeAutoRunRoundSummaries(summaryTotalRuns, roundSummaries),
      });
      await logAutoRunFinalSummary(summaryTotalRuns, roundSummaries);

      const finalRuntime = runtime.get();
      if (deps.getStopRequested() || stoppedEarly) {
        await addLog(
          targetSuccessMode
            ? `=== 已停止，成功导出 ${successfulRuns}/${finalRuntime.autoRunTotalRuns} 个 ===`
            : `=== 已停止，完成 ${successfulRuns}/${finalRuntime.autoRunTotalRuns} 轮 ===`,
          'warn'
        );
        await broadcastAutoRunStatus('stopped', {
        ...
        await addLog(
          targetSuccessMode
            ? `=== 目标完成：已成功导出 ${successfulRuns}/${finalRuntime.autoRunTotalRuns} 个 JSON ===`
            : `=== 全部 ${finalRuntime.autoRunTotalRuns} 轮已执行完成，成功 ${successfulRuns} 轮 ===`,
          'ok'
        );
