(function attachBackgroundLocalCliProxyApi(root, factory) {
  ...
  async function buildCredentialFileName(authJson, options = {}) {
    ...
    const prefix = normalizeString(options.fileNamePrefix || 'codex') || 'codex';
    if (!planType) {
      return `${prefix}-${email}.json`;
    }
    if (planType === 'team') {
      return `${prefix}-${hashAccountId}-${email}-${planType}.json`;
    }
    return `${prefix}-${email}-${planType}.json`;
  }
  ...
  function createLocalCliProxyApi(deps = {}) {
    ...
    async function buildAuthJsonArtifact(options = {}) {
      ...
      const outputDir = normalizeString(options.outputDir || options.outputDirectoryPath);
      const pluginDir = normalizeString(options.pluginDir);
      if (!outputDir && !pluginDir) {
        throw new Error('生成本地 auth json 失败：缺少 outputDir 或 pluginDir。');
      }

      const relativeAuthDir = outputDir ? '' : sanitizeRelativeDir(options.relativeAuthDir || DEFAULT_RELATIVE_AUTH_DIR);
      const directoryPath = outputDir || joinPath(pluginDir, relativeAuthDir);
    ...
    async function exchangeCallbackToAuthArtifact(options = {}) {
      ...
      return buildAuthJsonArtifact({
        ...
        outputDir: options.outputDir || options.outputDirectoryPath,
