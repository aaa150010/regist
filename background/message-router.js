    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'AUTO_RUN': {
          ...
          const state = await getState();
          if (String(state?.panelMode || '').trim() === 'existing-plus-oauth-json') {
            await setState({
          ...
          const targetSuccessMode = Boolean(message.payload?.targetSuccessMode);
          ...
          startAutoRunLoop(totalRuns, { autoRunSkipFailures, autoRunRetryNonFreeTrial, autoRunRetryPaypalCallback, targetSuccessMode, mode });
          return { ok: true };
        }
        ...
        case 'SCHEDULE_AUTO_RUN': {
          ...
          return await scheduleAutoRun(totalRuns, {
            ...
            targetSuccessMode: Boolean(message.payload?.targetSuccessMode),
        ...
        case 'SAVE_SETTING': {
          ...
          const profileRunnerProxyApplyRequested = (
            message.source === 'profile-runner'
            || Boolean(updates.profileRunnerProxyLocked)
          )
            && hasIpProxyUpdates
            && nextIpProxyEnabled;
          // 侧边栏仍仅在“手动开关代理”时自动应用；profile-runner 的隔离
          // Profile 每轮启动都必须重新应用一次，避免复用上轮持久化状态。
          const shouldApplyIpProxyOnSave = hasIpProxyUpdates
            && (
              profileRunnerProxyApplyRequested
              || (
        ...
        case 'PROBE_IP_PROXY_EXIT': {
          ...
          const result = await probeIpProxyExit({
            ...
            avoidOpenAiEndpoints: Boolean(message.payload?.avoidOpenAiEndpoints),
            skipTargetReachability: Boolean(message.payload?.skipTargetReachability),
          });
          return { ok: true, ...result };
        }
