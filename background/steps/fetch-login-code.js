(function attachBackgroundStep8(root, factory) {
  ...
  function createStep8Executor(deps = {}) {
    const {
      ...
      LUCKMAIL_PROVIDER,
      markCurrentRegistrationAccountUnavailable = null,
    ...
    function isExistingPlusOAuthJsonMode(state = {}) {
      return String(state?.panelMode || '').trim() === 'existing-plus-oauth-json';
    }

    function isAccountDeactivatedAuthState(pageState = {}) {
      return String(pageState?.state || '').trim() === 'account_deactivated_page'
        || /account_deactivated/i.test(String(pageState?.url || ''));
    }

    function isAccountDeactivatedError(error) {
      return /ACCOUNT_DEACTIVATED::|account_deactivated|账号已被删除或停用|账户已被删除或停用/i.test(
        String(typeof error === 'string' ? error : error?.message || '')
      );
    }

    function formatAuthStateLabel(pageState = {}) {
      if (typeof getLoginAuthStateLabel === 'function') {
        return getLoginAuthStateLabel(pageState?.state || pageState);
      }
      return String(pageState?.state || 'unknown');
    }

    async function throwForFatalAuthStateIfNeeded(pageState = {}, state = {}, visibleStep = 8) {
      if (!isAccountDeactivatedAuthState(pageState)) {
        return;
      }
      ...
      if (typeof markCurrentRegistrationAccountUnavailable === 'function') {
        await markCurrentRegistrationAccountUnavailable(latestState, {
          logPrefix: '检测到 OpenAI 账号已删除或停用',
          level: 'warn',
      ...
      throw new Error(
        `ACCOUNT_DEACTIVATED::步骤 ${visibleStep}：当前 OpenAI 账号已被删除或停用，已标记当前邮箱账号为已用，请自动切换下一个账号。当前状态：${formatAuthStateLabel(pageState)}。URL: ${pageState?.url || ''}`.trim()
      );
    }
    ...
    async function getLoginAuthStateFromContent(visibleStep, options = {}) {
      ...
      const latestState = typeof getState === 'function'
        ...
        : {};
      await throwForFatalAuthStateIfNeeded(result || {}, latestState, visibleStep);
      return result || {};
    }
    ...
    async function pollEmailVerificationCode(preparedState, pageState, visibleStep, runtime = {}) {
      ...
      const fastHotmailLoginPollOptions = mail.provider === HOTMAIL_PROVIDER && isExistingPlusOAuthJsonMode(preparedState)
        ? {
            initialDelayMs: 1500,
            initialPollMaxAttempts: 10,
            pollIntervalMs: 3000,
            minPollingResponseTimeoutMs: 12000,
          }
        : {};
    ...
    async function runStep8Attempt(state, runtime = {}) {
      ...
      await throwForFatalAuthStateIfNeeded(pageState, state, visibleStep);
      if (pageState?.state === 'oauth_consent_page') {
      ...
      while (true) {
        try {
          ...
          if (isAccountDeactivatedError(err)) {
            const latestState = typeof getState === 'function'
              ? await getState().catch(() => currentState)
              : currentState;
            if (typeof markCurrentRegistrationAccountUnavailable === 'function') {
              await markCurrentRegistrationAccountUnavailable(latestState, {
                logPrefix: '检测到 OpenAI 账号已删除或停用',
