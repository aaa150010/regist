(function attachBackgroundStep7(root, factory) {
  ...
  function createStep7Executor(deps = {}) {
    ...
    function isRateLimitExceededError(error) {
      return /RATE_LIMIT_EXCEEDED::|rate_limit_exceeded|请求过多|too\s+many\s+requests|rate\s+limit/i.test(
        String(typeof error === 'string' ? error : error?.message || '')
      );
    }
      ...
      while (attempt < STEP6_MAX_ATTEMPTS) {
        ...
        try {
          ...
          if (isAccountDeactivatedError(err)) {
            ...
            throw new Error(`ACCOUNT_DEACTIVATED::当前 OpenAI 账号已被删除或停用，已标记当前邮箱账号为已用，请重新获取下一个账号。`);
          }
