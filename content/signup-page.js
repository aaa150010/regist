...
const ACCOUNT_DEACTIVATED_PATTERN = /account_deactivated|账户已被删除或停用|账号已被删除或停用|该账户已被删除或停用|该账号已被删除或停用|you\s+do\s+not\s+have\s+an?\s+account|account\s+(?:has\s+been\s+)?(?:deleted|disabled|deactivated|suspended)/i;
const RATE_LIMIT_EXCEEDED_PATTERN = /rate_limit_exceeded|请求过多|请稍后重试|too\s+many\s+requests|rate\s+limit/i;
...
function isAccountDeactivatedPage() {
  return ACCOUNT_DEACTIVATED_PATTERN.test(getPageTextSnapshot());
}

function isRateLimitExceededPage() {
  return RATE_LIMIT_EXCEEDED_PATTERN.test(getPageTextSnapshot());
}
...
function inspectLoginAuthState() {
  ...
  const accountDeactivatedPage = isAccountDeactivatedPage();
  const rateLimitExceededPage = isRateLimitExceededPage();
  ...
  const baseState = {
    ...
    rateLimitExceededPage,
  };

  if (rateLimitExceededPage) {
    return {
      ...baseState,
      state: 'rate_limit_exceeded_page',
    };
  }

  if (accountDeactivatedPage) {
    return {
      ...baseState,
...
function throwForStep6FatalState(snapshot, visibleStep = 7, options = {}) {
  snapshot = normalizeStep6Snapshot(snapshot);
  const allowAddPhonePage = Boolean(options.allowAddPhonePage);
  switch (snapshot?.state) {
    case 'oauth_consent_page':
      return;
    case 'account_deactivated_page':
      throw new Error(`ACCOUNT_DEACTIVATED::当前账号已被删除或停用。URL: ${snapshot.url || location.href}`);
    case 'rate_limit_exceeded_page':
      throw new Error(`RATE_LIMIT_EXCEEDED::认证页请求过多，请稍后重试。URL: ${snapshot.url || location.href}`);
    case 'add_phone_page':
      if (allowAddPhonePage) return;
...
async function waitForVerificationSubmitOutcome(step, timeout, options = {}) {
  ...
  while (Date.now() - start < resolvedTimeout) {
    ...
    throwForStep6FatalState(authSnapshot, step, {
      allowAddPhonePage: step === 8,
    });
  ...
  const finalAuthSnapshot = inspectLoginAuthState();
  throwForStep6FatalState(finalAuthSnapshot, step, {
    allowAddPhonePage: step === 8,
  });
