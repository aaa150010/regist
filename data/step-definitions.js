(function attachStepDefinitions(root, factory) {
  ...
  const EXISTING_PLUS_OAUTH_JSON_STEP_DEFINITIONS = [
    { id: 1, order: 10, key: 'existing-plus-prepare-oauth', title: '准备 Plus 邮箱 OAuth', sourceId: 'openai-auth', driverId: null, command: 'existing-plus-prepare-oauth' },
    { id: 2, order: 20, key: 'oauth-email-login', title: '邮箱验证码登录', sourceId: 'openai-auth', driverId: 'content/signup-page', command: 'oauth-login', executeKey: 'oauth-login' },
    { id: 3, order: 30, key: 'fetch-login-code', title: '获取邮箱登录验证码', sourceId: 'openai-auth', driverId: 'content/signup-page', command: 'submit-verification-code', mailRuleId: 'openai-login-code' },
    { id: 4, order: 40, key: 'phone-verification', title: '手机接码验证', sourceId: 'openai-auth', driverId: 'content/signup-page', command: 'post-login-phone-verification', executeKey: 'post-login-phone-verification' },
    { id: 5, order: 50, key: 'confirm-oauth', title: '确认 OAuth 授权', sourceId: 'openai-auth', driverId: 'content/signup-page', command: 'confirm-oauth' },
    { id: 6, order: 60, key: 'existing-plus-save-json', title: '保存 SUB2API JSON', sourceId: 'local-helper', driverId: null, command: 'existing-plus-save-json' },
  ];
  ...
  function getOpenAiModeStepDefinitions(options = {}) {
    ...
    if (panelMode === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE) {
      return EXISTING_PLUS_OAUTH_JSON_STEP_DEFINITIONS;
    }
    if (!isPlusModeEnabled(options)) {
      if (signupMethod === SIGNUP_METHOD_PHONE) {
  ...
  function cloneNodes(steps = [], options = {}, flowId = DEFAULT_ACTIVE_FLOW_ID) {
    ...
    return steps.map((step) => ({
      ...
      executeKey: String(step.executeKey || step.key || '').trim(),
