...
(function attachSmsBowerProvider(root, factory) {
  ...
  const DEFAULT_BASE_URL = 'https://smsbower.app/stubs/handler_api.php';
  ...
  const DEFAULT_COUNTRY_ORDER = Object.freeze([
  ...
  function resolveCountryCandidates(state = {}) {
    ...
    const normalizedFallback = normalizeSmsBowerCountryFallback(fallbackSource);
    const fallbackCandidates = normalizedFallback.length
      ? normalizedFallback
      : DEFAULT_COUNTRY_ORDER.slice(1);
    const seen = new Set([primary.id]);
    const candidates = [primary];
    fallbackCandidates.forEach((entry) => {
