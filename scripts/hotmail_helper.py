...
def resolve_non_overwrite_path(target_path):
...
def save_local_cpa_json(file_path, content, directory_path="", avoid_overwrite=False):
    target_path = Path(str(file_path or "").strip()).expanduser()
    if not str(target_path):
        raise RuntimeError("Missing filePath")

    if not target_path.is_absolute():
        target_path = (PROJECT_ROOT_DIR / target_path).resolve()

    if directory_path:
        target_dir = Path(str(directory_path).strip()).expanduser()
        if not target_dir.is_absolute():
            target_dir = (PROJECT_ROOT_DIR / target_dir).resolve()
        target_dir.mkdir(parents=True, exist_ok=True)
        target_path = target_dir / target_path.name

    target_path.parent.mkdir(parents=True, exist_ok=True)
    if avoid_overwrite:
        target_path = resolve_non_overwrite_path(target_path)
        ...
        try:
            ...
            if request_path == "/save-auth-json":
                file_path = save_local_cpa_json(
                    ...
                    bool(payload.get("avoidOverwrite")),
                )
                json_response(self, 200, {
                    "ok": True,
                    "filePath": file_path,
