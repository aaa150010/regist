...
const rowExistingPlusJsonOutputDir = document.getElementById('row-existing-plus-json-output-dir');
const inputExistingPlusJsonOutputDir = document.getElementById('input-existing-plus-json-output-dir');
const rowExistingPlusAccounts = document.getElementById('row-existing-plus-accounts');
const inputExistingPlusAccounts = document.getElementById('input-existing-plus-accounts');
...
function normalizeExistingPlusJsonOutputDirInput(value = '') {
  ...
  if (/^\//.test(trimmed) || /^[A-Za-z]:[\\/]/.test(trimmed) || trimmed) {
    return DEFAULT_EXISTING_PLUS_JSON_OUTPUT_DIR;
  }
  return DEFAULT_EXISTING_PLUS_JSON_OUTPUT_DIR;
}
...
const DEFAULT_FIVE_SIM_COUNTRY_ID = 'colombia';
const DEFAULT_FIVE_SIM_COUNTRY_LABEL = '哥伦比亚 (Colombia)';
const FIVE_SIM_SUPPORTED_COUNTRY_ITEMS = Object.freeze([
  { id: 'colombia', chn: '哥伦比亚', eng: 'Colombia', searchText: 'colombia 哥伦比亚 Colombia CO +57' },
  ...
  { id: 'southafrica', chn: '南非', eng: 'South Africa', searchText: 'southafrica 南非 South Africa ZA +27' },
  { id: 'philippines', chn: '菲律宾', eng: 'Philippines', searchText: 'philippines 菲律宾 Philippines PH +63' },
  ...
  { id: 'sweden', chn: '瑞典', eng: 'Sweden', searchText: 'sweden 瑞典 Sweden SE +46' },
  { id: 'portugal', chn: '葡萄牙', eng: 'Portugal', searchText: 'portugal 葡萄牙 Portugal PT +351' },
...
function collectSettingsPayload() {
  ...
  const normalizePanelModeSafe = typeof normalizePanelMode === 'function'
    ...
    : ((value = '') => {
      const normalized = String(value || '').trim().toLowerCase();
      return normalized === 'existing-plus-oauth-json'
        ? normalized
        : DEFAULT_PANEL_MODE;
    });
  ...
  const fixedPlusModeEnabled = typeof FIXED_PLUS_MODE_ENABLED === 'boolean'
    ...
    : true;
  const existingPlusModeSelected = normalizePanelMode(effectivePanelMode) === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE;
  ...
  return {
    ...
    mailProvider: existingPlusModeSelected
      ? HOTMAIL_PROVIDER
      : supportedMailProviderNormalizer(selectMailProvider?.value || latestState?.mailProvider),
    mail2925Mode: getSelectedMail2925Mode(),
    mail2925UseAccountPool,
    currentMail2925AccountId: String(latestState?.currentMail2925AccountId || '').trim(),
    emailGenerator: existingPlusModeSelected ? 'duck' : selectEmailGenerator.value,
...
async function loadHeroSmsCountries() {
  ...
  if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
    ...
    try {
      ...
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 10000);
      const url = new URL(DEFAULT_SMSBOWER_BASE_URL);
      url.searchParams.set('action', 'getCountries');
      url.searchParams.set('api_key', apiKey);
...
async function previewPhoneSmsBalance() {
  ...
  try {
    ...
    if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
      ...
      url = new URL(DEFAULT_SMSBOWER_BASE_URL);
    } else if (provider === PHONE_SMS_PROVIDER_SMS_VERIFICATION_NUMBER) {
...
function syncStepDefinitionsForMode(plusModeEnabled = false, plusPaymentMethodOrOptions = {}, maybeOptions = {}) {
  ...
  const useExistingPlusWorkflow = nextPanelMode === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE;
  ...
  const currentlyUsingExistingPlusWorkflow = (typeof workflowNodes !== 'undefined' ? workflowNodes : [])
    .some((node) => String(node?.nodeId || '').trim() === 'existing-plus-prepare-oauth');
  const noRtWorkflowModeChanged = useNoRtWorkflow !== currentlyUsingNoRtWorkflow;
  const existingPlusWorkflowModeChanged = useExistingPlusWorkflow !== currentlyUsingExistingPlusWorkflow;
  ...
  const shouldRender = Boolean(options.render)
    ...
    || existingPlusWorkflowModeChanged
    || paymentTitleChanged;
  if (!shouldRender) {
    return;
  }
  ...
  rebuildStepDefinitionState(nextPlusModeEnabled, {
    activeFlowId: nextActiveFlowId,
    ...((useNoRtWorkflow || useExistingPlusWorkflow) ? { panelMode: nextPanelMode } : {}),
    plusPaymentMethod: nextPaymentMethod,
...
function applySettingsState(state) {
  ...
  if (typeof inputExistingPlusAccounts !== 'undefined' && inputExistingPlusAccounts) {
    inputExistingPlusAccounts.value = state?.existingPlusAccountsText || '';
  }
  if (typeof inputExistingPlusJsonOutputDir !== 'undefined' && inputExistingPlusJsonOutputDir) {
    inputExistingPlusJsonOutputDir.value = normalizeExistingPlusJsonOutputDirInput(state?.existingPlusJsonOutputDir);
  }
...
function updateMailProviderUI() {
  ...
  if (selectMailProvider && selectMailProvider.value !== HOTMAIL_PROVIDER) {
    selectMailProvider.value = HOTMAIL_PROVIDER;
  }
...
function updatePanelModeUI() {
  ...
  if (rowAccountAccessStrategy) {
    rowAccountAccessStrategy.style.display = exportTarget === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE ? 'none' : '';
  }
  ...
  const useExistingPlusOauthJson = panelMode === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE;
  ...
  setRowDisplay(rowExistingPlusJsonOutputDir, false);
  setRowDisplay(rowExistingPlusAccounts, false);
  if (!useLocalCpaJson) {
  ...
  if (step9Btn) {
    step9Btn.textContent = useExistingPlusOauthJson
      ? '保存 SUB2API JSON'
      : (useLocalCpaJson
...
function validateExistingPlusOauthJsonSettings(options = {}) {
  const panelMode = options?.panelMode
    || (typeof getSelectedPanelMode === 'function' ? getSelectedPanelMode() : latestState?.panelMode);
  if (normalizePanelMode(panelMode) !== EXISTING_PLUS_OAUTH_JSON_PANEL_MODE) {
    return { valid: true, required: false };
  }
  const outputDir = DEFAULT_EXISTING_PLUS_JSON_OUTPUT_DIR;
  const hotmailAccounts = typeof getHotmailAccounts === 'function'
    ? getHotmailAccounts(latestState)
    : (Array.isArray(latestState?.hotmailAccounts) ? latestState.hotmailAccounts : []);
  const hasAccount = hotmailAccounts.some((account) => (
    account
    && (account.status === 'authorized' || String(account.refreshToken || '').trim())
    && !account.used
    && String(account.refreshToken || '').trim()
  ));
  const valid = hasAccount;
  if (inputExistingPlusJsonOutputDir) {
    inputExistingPlusJsonOutputDir.value = DEFAULT_EXISTING_PLUS_JSON_OUTPUT_DIR;
    inputExistingPlusJsonOutputDir.classList.remove('is-invalid');
    inputExistingPlusJsonOutputDir.title = '';
  }
  if (inputExistingPlusAccounts) {
    inputExistingPlusAccounts.classList.remove('is-invalid');
    inputExistingPlusAccounts.title = '';
  }
  return {
    valid,
    required: true,
    outputDir,
    accountsText: '',
    message: !hasAccount ? '请先在微软邮箱账户池导入至少一个已授权且未使用的账号。' : '',
  };
}
...
[inputExistingPlusJsonOutputDir, inputExistingPlusAccounts].forEach((input) => {
  input?.addEventListener('input', () => {
    validateExistingPlusOauthJsonSettings();
    markSettingsDirty(true);
...
async function importPendingHotmailTextBeforeStart() {
  const rawText = String(inputHotmailImport?.value || '').trim();
  if (!rawText) {
    return 0;
  }
  if (typeof parseHotmailImportText !== 'function') {
    throw new Error('微软邮箱账户池导入解析器未加载，请刷新扩展后重试。');
  }
  const parsedAccounts = parseHotmailImportText(rawText);
  if (!parsedAccounts.length) {
    throw new Error('微软邮箱账户池导入框里没有解析到有效账号，请检查格式是否为 email----clientId----mailRefreshToken 或 email----password----clientId----refreshToken。');
  }
  ...
  const refreshedState = await chrome.runtime.sendMessage({ type: 'GET_STATE', source: 'sidepanel' });
  if (refreshedState && !refreshedState.error) {
    syncLatestState(refreshedState);
  }
  renderHotmailAccounts();
  showToast(`启动前已自动导入 ${importedCount} 条微软邮箱账号`, 'success', 2200);
...
async function startAutoRunFromCurrentSettings() {
  ...
  await importPendingHotmailTextBeforeStart();
  ...
  const existingPlusValidation = validateExistingPlusOauthJsonSettings();
  if (!existingPlusValidation.valid) {
    clearPendingAutoRunStartRunCount();
    inputExistingPlusAccounts?.focus?.();
    throw new Error(existingPlusValidation.message || '已有 Plus OAuth JSON 配置不完整。');
  }
  const existingPlusMode = normalizePanelMode(
    typeof getSelectedPanelMode === 'function' ? getSelectedPanelMode() : latestState?.panelMode
  ) === EXISTING_PLUS_OAUTH_JSON_PANEL_MODE;
  if (!existingPlusMode) {
    const hostedCheckoutValidation = validateHostedCheckoutContactConfig({ focusOnError: true });
  ...
  const response = await sendSidepanelMessage({
    ...
    payload: {
      ...
      targetSuccessMode: existingPlusMode,
      contributionMode: Boolean(latestState?.contributionMode),
  ...
  switch (message.type) {
    ...
    case 'AUTO_RUN_RESET': {
      ...
      syncLatestState({
        ...
        lastLoginCode: null,
        localhostUrl: null,
        existingPlusCurrentAccountId: null,
        existingPlusCurrentEmail: '',
